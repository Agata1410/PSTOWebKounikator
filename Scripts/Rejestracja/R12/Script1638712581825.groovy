...
for (def row = 1; row <= findTestData('Rejestracja/tabR12').getRowNumbers(); row++) {
    ...
    WebUI.setText(findTestObject('Page_PSTO webMessenger/input_Zarejestruj konto_passwordLogin'), findTestData('Rejestracja/tabR12').getValue(
            'password', row))

    WebUI.click(findTestObject('Page_PSTO webMessenger/input_Zarejestruj konto_login'), FailureHandling.CONTINUE_ON_FAILURE)

    WebUI.takeScreenshot(data + 'bugR12.png')
